...
select name as '学生的姓名' ,phone  as '学生的电话'
from student 
where studentno 
in (10110110004,10110110005,10110110006);

select * from student order by gradeid,birthdate desc;
select Concat(Host,' ',User)  as USER from mysql.user;
select concat(phone," ",address) as '联系方式' from student;


select gradeid from grade where gradename='s28';-- 28
select * from student where gradeid=28;

select name,phone from student where gradeid=28;

select * from student where gender='女' and gradeid=28;

select * from subject where classhours>=60;

create table stu(id int primary key,name varchar(50));

insert into stu values(1,'点击'),(2,'刘华'),(3,'得分'),(4,null),(5,'昌华');
select * from stu;
...
select * from subject;
select gradeid from grade where gradename='s29';
select * from subject where gradeid=29;

select * from student  where gender='男' and gradeid=29;
select name ,address from student where gender='男' and gradeid=29;


select student.name,grade.*
from student,grade
where student.birthdate>'1990-01-01' and grade.gradename='s29'
